...
!       end if
  ...
  subroutine SetMaxCondConnections(site_hydr, cohort_hydr, h_node, kmax_dn, kmax_up, hard_level, bc_in)
       ...
       do k = 1, n_hypool_aroot + nshell
          ...
          elseif( k == 2) then ! aroot-soil
             ...
             if (hlm_use_hydrohard .eq. itrue) then
                ...
                !  if (soil_rate_temporary<hard_rate_temporary) then
                !    kmax_up(icnx)=kmax_up(icnx)*10**soil_rate_temporary 
                !!    if ((bc_in%t_soisno_sl(j)-tfrz)<=-15._r8) then 
